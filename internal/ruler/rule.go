...
/*
This struct is the main format for rules or conditions in context-compatable libraries.
...
but go-context has a facility to help decode your rules from JSON into its own structs.
*/
type Rule struct {
...
/*
A RulerRule combines a single context and a whole set of rules and is used
...
// Stops chaining for the current context, allowing you
// to add rules for other properties
func (rf *RulerRule) End() *Ruler {
	return rf.Ruler
}

// comparator will either create a new context filter and add its filter
func (rf *RulerRule) compare(comp int, value interface{}) *RulerRule {
	...
	// if this thing has a comparator already, we need to make a new context filter
	if rf.Comparator != "" {
		rf = &RulerRule{
			rf.Ruler,
			&Rule{
		...
		// attach the new filter to the context
